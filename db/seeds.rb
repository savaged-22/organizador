...
#Area para crear emails de users.
users = ['juan', 'andrea', 'leon', 'andres', 'natalia', 'camilo', 'rusbel', 'johan']
users.each do |name|
    User.create email: "#{name}@platzi.com", password: '12345678'
end
puts 'Users has been created'



#Area para crear cateogias.
...
#Crear tareas y participantes.
owner = User.find_by(email: 'juan@platzi.com') #El correo encargado de manejar las tareas y participantes.
...
['conceptualización', 'Conclusiones del curso', ['juan:1', 'leon:2', 'andrea:random']],
    ...
    participants = participant_set.map do |participant|
        ...
        Participant.new(user:User.find_by(email: "#{user_name}@platzi.com"),
        role: role.to_i
        )
    end
    Task.create!(
